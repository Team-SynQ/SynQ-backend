...
class DocumentChunkSearchQueryTest extends PostgresTestContainer {
	...
	private ReferenceMaterialTestFixture.Fixture referenceA;
	private ReferenceMaterialTestFixture.Fixture referenceB;
	...
	void setUp() {
		...
		referenceA = referenceMaterialTestFixture.create();
		referenceB = referenceMaterialTestFixture.create();
	}
	...
	void 다른_프로젝트의_청크는_검색되지_않는다() {
		...
		repository.save(DocumentChunk.of(
				referenceA.referenceMaterialId(),
				referenceA.projectId(),
				0,
				"A 프로젝트 문서",
				vector(1.0f, 0.0f),
				MODEL
		...
		repository.save(DocumentChunk.of(
				referenceB.referenceMaterialId(),
				referenceB.projectId(),
				0,
				"B 프로젝트 문서",
				vector(1.0f, 0.0f),
				MODEL
		...
		List<ChunkSearchRow> rows = repository.searchByProject(
				referenceA.projectId(), queryVector(), -1.0, 10);
	...
	void 유사도가_높은_순으로_정렬된다() {
		repository.save(DocumentChunk.of(
				referenceA.referenceMaterialId(), referenceA.projectId(),
				0, "유사도 0.6", vector(0.6f, 0.8f), MODEL));
		repository.save(DocumentChunk.of(
				referenceA.referenceMaterialId(), referenceA.projectId(),
				1, "유사도 1.0", vector(1.0f, 0.0f), MODEL));
		repository.save(DocumentChunk.of(
				referenceA.referenceMaterialId(), referenceA.projectId(),
				2, "유사도 0.8", vector(0.8f, 0.6f), MODEL));
		repository.flush();
	...
	void 임계값_미만은_제외한다() {
		...
		List<ChunkSearchRow> rows = repository.searchByProject(
				referenceA.projectId(), queryVector(), 0.7, 10);
	...
	void topK_개수만큼만_반환한다() {
		...
		List<ChunkSearchRow> rows = repository.searchByProject(
				referenceA.projectId(), queryVector(), -1.0, 2);
	...
	void 청크가_없는_프로젝트는_빈_결과다() {
		repository.save(DocumentChunk.of(
				referenceA.referenceMaterialId(), referenceA.projectId(),
				0, "청크", vector(1.0f, 0.0f), MODEL));
		repository.flush();

		List<ChunkSearchRow> rows = repository.searchByProject(
				referenceB.projectId(), queryVector(), -1.0, 10);
