...
public interface MeetingControllerDocs {
	...
	@Operation(summary = "회의 생성", description = "생성과 동시에 녹음이 시작되며(IN_PROGRESS), 참여 동의(consentAgreed)가 필요하다. "
			+ "wsUrl은 요청 scheme에 맞춰 ws:// 또는 wss:// 절대 URL로 내려간다.")
	@ApiResponses(@io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "201", description = "회의 생성 성공"))
	@PostMapping
	...
	@SecurityRequirement(name = "bearerAuth")
	@Operation(summary = "회의 목록 조회", description = "프로젝트의 회의 목록을 생성일 최신순으로 조회한다. "
			+ "keyTopics는 요약이 아직 생성되지 않았으면 null, 요약은 있으나 주제가 없으면 빈 배열이다. "
			+ "durationSeconds는 회의가 아직 진행 중이면 null이다.")
	@ApiResponses(@io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "회의 목록 조회 성공"))
	@GetMapping
