package com.synq.backend.domain.project.code;

import com.synq.backend.global.apipayload.code.BaseCode;
import lombok.AllArgsConstructor;
import lombok.Getter;
import org.springframework.http.HttpStatus;

@Getter
@AllArgsConstructor
public enum ProjectErrorCode implements BaseCode {
	CANNOT_DELETE_PROJECT_OWNER(HttpStatus.BAD_REQUEST,
			"PROJECT400_1",
			"프로젝트 소유자는 삭제할 수 없습니다."),
	PROJECT_OWNER_CANNOT_LEAVE(HttpStatus.BAD_REQUEST,
			"PROJECT400_2",
			"프로젝트 소유자는 프로젝트에서 나갈 수 없습니다."),
	INVALID_JOIN_REQUEST_ROLE_SETTING(HttpStatus.BAD_REQUEST,
			"PROJECT400_3",
			"참여 요청의 역할·관점 설정이 올바르지 않습니다."),
	INVALID_PROJECT_ROLE_PERSPECTIVE(HttpStatus.BAD_REQUEST,
			"PROJECT400_4",
			"프로젝트 역할·관점 설정이 올바르지 않습니다."),
	NOT_PROJECT_OWNER(HttpStatus.FORBIDDEN,
			"PROJECT403_1",
			"프로젝트 소유자만 초대 링크를 생성할 수 있습니다."),
	NOT_PROJECT_MEMBER(HttpStatus.FORBIDDEN,
			"PROJECT403_2",
			"프로젝트 접근 권한이 없습니다."),
	USER_NOT_FOUND(HttpStatus.NOT_FOUND,
			"PROJECT404_1",
			"사용자를 찾을 수 없습니다."),
	INVITATION_NOT_FOUND(HttpStatus.NOT_FOUND,
			"PROJECT404_2",
			"초대 정보를 찾을 수 없습니다."),
	PROJECT_NOT_FOUND(HttpStatus.NOT_FOUND,
			"PROJECT404_3",
			"프로젝트를 찾을 수 없습니다."),
	PROJECT_MEMBER_NOT_FOUND(HttpStatus.NOT_FOUND,
			"PROJECT404_4",
			"프로젝트 멤버를 찾을 수 없습니다."),
	INVITATION_PROJECT_MISMATCH(HttpStatus.NOT_FOUND,
			"PROJECT404_5",
			"프로젝트에 해당하는 초대 정보를 찾을 수 없습니다."),
	JOIN_REQUEST_NOT_FOUND(HttpStatus.NOT_FOUND,
			"PROJECT404_6",
			"프로젝트 참여 요청을 찾을 수 없습니다."),
	USER_PROJECT_LIMIT_EXCEEDED(HttpStatus.CONFLICT,
			"PROJECT409_1",
			"사용자별 최대 프로젝트 수를 초과했습니다."),
	PROJECT_MEMBER_LIMIT_EXCEEDED(HttpStatus.CONFLICT,
			"PROJECT409_2",
			"프로젝트 최대 인원을 초과했습니다."),
	ALREADY_PROJECT_MEMBER(HttpStatus.CONFLICT,
			"PROJECT409_3",
			"이미 참여 중인 프로젝트입니다."),
	JOIN_REQUEST_ALREADY_EXISTS(HttpStatus.CONFLICT,
			"PROJECT409_4",
			"이미 승인 대기 중인 참여 요청이 있습니다."),
	JOIN_REQUEST_ALREADY_PROCESSED(HttpStatus.CONFLICT,
			"PROJECT409_5",
			"이미 처리된 프로젝트 참여 요청입니다."),
	INVITATION_EXPIRED(HttpStatus.GONE,
			"PROJECT410_1",
			"초대 링크가 만료되었습니다.");

	private final HttpStatus status;
	private final String code;
	private final String message;
}
